create or replace function public.customer_register_device(_device_id text, _device_label text default null)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if v_uid is null or v_type is distinct from 'customer' then
...
create or replace function public.customer_list_devices()
returns jsonb
language plpgsql
stable
security definer
...
create or replace function public.customer_revoke_device(_device_id text)
returns void
language plpgsql
security definer
set search_path to 'public'
...
revoke all on function public.customer_register_device(text, text) from public, anon;
revoke all on function public.customer_list_devices() from public, anon;
revoke all on function public.customer_revoke_device(text) from public, anon;
grant execute on function public.customer_register_device(text, text) to authenticated;
grant execute on function public.customer_list_devices() to authenticated;
grant execute on function public.customer_revoke_device(text) to authenticated;